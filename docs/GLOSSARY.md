...
| **TimelineClip** | The feature that draws a placed (persisted) `ProjectClip` on its track's `Swimlane`: it positions a `SwimlaneItem` from the clip's sample `start`/`duration` and renders a `standard`-variant **Clip (component)** labelled with the source audio's filename (or a generic fallback for recordings, which have none). Rendered per track by `TrackLanes`; pairs with `RecordingClip`, which shows the live take before it becomes one of these (`app/src/components/features/TimelineClip.tsx`). |
