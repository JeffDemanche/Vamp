...
| **Stacked clip** | A `ProjectClip` with `mode = STACKED`. Occupies one loop region on the timeline — normally its `duration`/`maxDuration` equal `ProjectAudio.loopLength` (`L`), but if a looped **Recording** stops before the first loop point, `duration`/`maxDuration` match the captured length instead (still `STACKED`, with `loopLength` stored on the audio). The underlying recording spans one or more loop passes. On playback the engine overlays every pass on top of each other within that region: it schedules one audio event per pass, all starting at the clip's `start` (truncated at the clip end) but reading a different `L`-sized slice of the recording (`bufferOffset += k*L`). The number of passes is the recording's length ÷ `L`. Because all passes anchor to `start`, shortening the clip below `L` keeps them in sync. Produced automatically for looped **Recording**s. |
...
| `loopLength` | `Int` | The loop length (in samples) active when this audio was recorded over a looping transport. `null` for non-looped takes and file imports. Used by **stacked** clips to re-trigger at every loop point. |
...
| **AudioEngine** | The client-side playback **and recording** engine: the interface between the editor UI, the Web Audio API, the decoded audio files it holds in memory (keyed by `ProjectAudio._id`), and the **audio events** derived from a project's clips. `loadAudio`/`setAudioBuffer` populate the in-memory store; upstream code fetches `READY` audio bytes via `ensureAudioLoaded` before scheduling. The decoded buffers are also exposed for visualisation (`getAudioBuffer`, plus a `subscribeAudioStore` change notification) so the UI can derive a **Clip waveform** without re-downloading the file. During capture it also taps the mic stream into an accumulating PCM buffer (`getRecordingBuffer`, `subscribeRecordingBuffer`) resampled to the timeline sample rate for the live **RecordingClip** waveform. `update` reflects the editor state it depends on (clips plus the timeline's sample rate and **Playback range**, including `loop`) and re-derives its events — **flat** clips produce one event per clip; **stacked** clips produce one event per recorded loop pass, overlaid within the clip's single loop region (all sharing the clip window but reading successive `loopLength`-sized slices of the recording, truncated at the clip end), with the pass count taken from the decoded buffer length so it settles once the audio loads. `play`/`stop` begin/end playback by scheduling each event as a Web Audio source against the audio clock; with `loop` on it restarts at `playStart` when it reaches `playEnd` instead of stopping. It tracks the playing state and the audio-clock/sample anchors so its `timecode` getter reports the current timeline position (in samples) accurately for the UI to poll. For capture, `startRecording` acquires the microphone (via an injectable `MediaStreamProvider`) and records with `MediaRecorder`, resolving once capture truly begins with the timeline `startSample` it is anchored to (and, with `startPlayback`, begins playback in that same audio-clock instant so the take and the playhead share one anchor rather than the clip drifting ahead of a later, separate `play`); when the transport is looping it also captures `loopLength` on the resolve value and `RecordingResult`. `stopRecording` resolves with a `RecordingResult` (the audio `Blob`, its `contentType`, the `startSample`/`durationSamples` placement, and optional `loopLength`, the duration measured on the audio clock) (`app/src/audio/AudioEngine.ts`). |
| **AudioEngineProvider** | The client glue that owns one `AudioEngine` per timeline editor and keeps it in sync with the editor's jotai state and the project's content. Rendered inside the editor's jotai `Provider`; reads the cached `project` query, maps each `READY` `ProjectClip` into engine clips, and downloads the project's whole audio library (`projectData.audios`) via `ensureAudioLoaded` into the engine's in-memory store (keyed by `ProjectAudio._id`) — rather than walking clips — before/while pushing clips plus the sample rate and **Playback range** into `engine.update`. Exposes `useAudioEngine` (the instance), `useAudioEnginePlaying` (reactive playing state), `useAudioEngineTimecode` (the live `timecode`, polled on `requestAnimationFrame` for the playhead), `useAudioBuffer` (a clip's decoded buffer, re-rendering when it lands, for the **Clip waveform**), and `useRecordingBuffer` (the in-progress take's PCM, for the live **RecordingClip** waveform) (`app/src/audio/AudioEngineProvider.tsx`). |
...
| **Clip mode badge** | A subtle icon in a **Clip (component)** header indicating its **Clip mode**, built by `ClipModeBadge` and passed into the clip's `badge` slot (`app/src/components/features/TimelineClip.tsx`). Flat clips show a single-rectangle glyph; stacked clips show a stack glyph plus the number of audio events dispatched — the count of recorded loop passes overlaid in the clip, computed from the decoded recording's length ÷ its loop length via the engine's shared `stackedLayerCount`, so it matches the events the **AudioEngine** schedules. The count appears once the audio buffer has loaded. Also drawn on the **Clip drag** preview. |
| **Clip waveform** | The waveform drawn in the background of a **Clip (component)**. The `ClipWaveform` feature reduces a clip's `[audioOffset, audioOffset + duration)` window to mono peaks (`computeClipPeaks`, capped at 8 000 points) and hands them to the pure **Waveform** primitive. For placed clips it reads the source audio from the `AudioEngine`'s decoded buffer (`useAudioBuffer`, no re-fetch); for the live **RecordingClip** it reads in-progress PCM from `useRecordingBuffer` and uses the `recording` colour variant (destructive foreground). When the clip is longer than the available audio, peaks and display width follow the audio length rather than stretching across the full clip. A **Stacked clip** (or looped live take) draws one overlaid waveform layer per recorded loop pass (count via the engine's `stackedLayerCount`), each reading the slice at that pass's buffer offset (`audioOffset + k*loopLength`) so the layers mirror the engine's stacked audio events, with each layer's alpha divided by the layer count so the stack reads at roughly a flat clip's opacity; a flat clip draws a single layer. Renders nothing until audio is available (`app/src/components/features/ClipWaveform.tsx`). |ecoded, so the waveform fades in once bytes land. Its colour tracks the clip's `selected`/`hovered` state to match the **Selectable surface** styling — a foreground-on-fill token when lit up, a subtle `--foreground` tint otherwise — resolved to a concrete `color-mix` value (canvas can't paint `var(...)`). Mounted by `TimelineClip` (and the **Clip drag** preview in `TrackLanes`) (`app/src/components/features/ClipWaveform.tsx`, `app/src/audio/waveformPeaks.ts`). |
...
| **RecordingClip** | The feature that draws the live, in-progress **Recording** as a red `recording`-variant **Clip (component)** on its track's `Swimlane`. It reads the active recording (`useRecording`) and how much audio the engine has captured on the audio clock (`useRecordingCapturedSamples`), placing a `SwimlaneItem` at the recording's `startSample`. For a flat take the clip grows with capture progress; for a looped take it grows until one loop length has been captured then locks to one loop region (via `recordingClipDisplay`, matching persisted clip placement) — using the audio clock so the width stays at full loop length after the transport wraps. A live **Clip waveform** is drawn from the engine's in-progress PCM tap (`useRecordingBuffer`), using the same stacked/flat layering rules as placed clips. Renders nothing unless a recording is active on its `trackId`, so it disappears when recording stops (a placed `standard` clip replaces it once the take is saved). Rendered inside each lane by `TrackLanes` (`app/src/components/features/RecordingClip.tsx`). |
| **RecordingController** | The feature that owns the editor's recording lifecycle and exposes it to the `TimelineToolbar` via `useRecordingControls`. Probes **Microphone access** on mount (no prompt) and tracks changes; `beginRecording` acquires the mic and starts capture on the `AudioEngine` *before* flipping recording state or starting playback (so no lead-in is dropped), surfacing denied/no-mic failures as a user-facing error. When playback stops mid-recording it stops capture, clears the live recording state (hiding the `RecordingClip`), and runs the **Audio upload flow** (`uploadAudioAndCreateClip`) to `createClip` with the captured audio anchored at the recording's `startSample` for its measured duration. Replaces the former headless `RecordingTransportController`. Mounted inside `TimelineEditor`'s `AudioEngineProvider`/jotai `Provider` (`app/src/components/features/RecordingController.tsx`). |
...
| **TimelineToolbar** | The feature toolbar above the timeline surface holding playback/timeline controls: a **loop** toggle (flips `loop` in the **Playback range**), a **play/stop** button that drives the editor's `AudioEngine` and reflects its playing state, and a **record** button that hands off to the `RecordingController` (`beginRecording`) — disabled until a track is selected, reflecting blocked **Microphone access**, and surfacing any capture error beside the toolbar. Rendered by `TimelineEditor` inside the jotai `Provider`/`AudioEngineProvider`/`RecordingController` (`app/src/components/features/TimelineToolbar.tsx`). |
