...
| **Selected clip** | The one `ProjectClip` the user has selected on the timeline. Client-only editor state (`selectedClipIdAtom` in `app/src/state/timeline.ts`, accessed via `useSelectedClip`); at most one clip is selected at a time, toggled by clicking a clip (`TimelineClip`). Not persisted (unlike the **Selected track**). |
...
| **TimelineClip** | The feature that draws a placed (persisted) `ProjectClip` on its track's `Swimlane`: it positions a `SwimlaneItem` from the clip's sample `start`/`duration` and renders a `standard`-variant **Clip (component)** labelled with the source audio's filename (or a generic fallback for recordings, which have none). Clicking it toggles the **Selected clip** (`useSelectedClip`). Rendered per track by `TrackLanes`; pairs with `RecordingClip`, which shows the live take before it becomes one of these (`app/src/components/features/TimelineClip.tsx`). |
